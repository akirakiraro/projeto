...
void formatar_cpf(const char *cpf_original, char *cpf_formatado) {
    ...
    snprintf(cpf_formatado, 15, "%c%c%c.%c%c%c.%c%c%c-%c%c",
             cpf_original[0], cpf_original[1], cpf_original[2],
             cpf_original[3], cpf_original[4], cpf_original[5],
             cpf_original[6], cpf_original[7], cpf_original[8],
...
int add_usuario (char cpf[12], int senha){
    ...
    login = fopen ("usuario.bin", "ab");                        // abre o arquivo em modo de "write binario"
    fwrite(&cpf, sizeof(int), 1, login);                        // escreve no arquivo o cpf
    fwrite(&senha, sizeof(int), 1, login);                      // escreve no arquivo a senha
    // como eu preciso ter um valor nessas variaveis e a conta é nova ele nao tem q comecar com dinheiros 
...
int ADM() {
    ...
    // Verifica se o arquivo existe; caso contrário, cria o arquivo com o usuário ADM
    if (login == NULL) {
        limpa();
    ...
    if (strcmp(temp.cpf, cpf_ADM) == 0 && temp.senha == senha_ADM) {
        ...
        // Se não for, reescreve o arquivo com o usuário ADM no início e os outros usuários
        login = fopen("usuario.bin", "rb");
        if (login == NULL) {
